...
const { ProcessRecords } = require("../../../doc-version/content-types/process_records")
...
<html lang="en">
  ...
  <body data-id="__react-email-body" style="background-color:#ffffff;font-family:HelveticaNeue,Helvetica,Arial,sans-serif">
    <table align="center" width="100%" data-id="__react-email-container" role="presentation" cellSpacing="0" cellPadding="0" border="0" style="max-width:37.5em;background-color:#ffffff;border:1px solid #eee;border-radius:5px;box-shadow:0 5px 10px rgba(20,50,70,.2);margin-top:20px;width:360px;margin:0 auto;padding:68px 1rem 70px">
      <tbody>
        <tr style="width:100%">
          <td><img data-id="react-email-img" alt="Plaid" src="https://easysign.rhinocerosoftware.co/images/logo.jpeg" width="70" style="display:block;outline:none;border:none;text-decoration:none;margin:0 auto" />
            ...
            <h1 data-id="react-email-heading" style="color:#000;display:inline-block;font-family:HelveticaNeue-Medium,Helvetica,Arial,sans-serif;font-size:16px;font-weight:300;line-height:24px;margin-bottom:0;margin-top:0;text-align:center">Mme / Mr <b>{{customerName}}</b>, votre collègue <b>{{author}}</b> a demandé votre signature pour le document <b>{{documentName}}</b></h1>
    ...
    <table align="center" width="100%" data-id="__react-email-container" role="presentation" cellSpacing="0" cellPadding="0" border="0" style="max-width:37.5em;background-color:#ffffff;border:1px solid #eee;border-radius:5px;box-shadow:0 5px 10px rgba(20,50,70,.2);margin-top:20px;width:360px;margin:0 auto;padding:68px 1rem 70px">
      <tbody>
        ...
        <td><img data-id="react-email-img" alt="Plaid" src="https://easysign.rhinocerosoftware.co/images/logo.jpeg" width="70" style="display:block;outline:none;border:none;text-decoration:none;margin:0 auto" />
            ...
            <h1 data-id="react-email-heading" style="color:#000;display:inline-block;font-family:HelveticaNeue-Medium,Helvetica,Arial,sans-serif;font-size:16px;font-weight:300;line-height:24px;margin-bottom:0;margin-top:0;text-align:center">Mme / Mr <b>{{rejectorName}}</b>, votre collègue <b>{{authorName}}</b> a rejété votre document <b>{{documentName}} </b>pour motif <b>{{rejectionMessage}}</b>.</h1>
            <table align="center" width="100%" data-id="react-email-section" style="background:#d8f7c6;border-radius:4px;margin:16px auto 14px;vertical-align:middle;width:280px" border="0" cellPadding="0" cellSpacing="0" role="presentation">
              <tbody>
                <tr>
                  <td>
    ...
    <table align="center" width="100%" data-id="__react-email-container" role="presentation" cellSpacing="0" cellPadding="0" border="0" style="max-width:37.5em;background-color:#ffffff;border:1px solid #eee;border-radius:5px;box-shadow:0 5px 10px rgba(20,50,70,.2);margin-top:20px;width:360px;margin:0 auto;padding:68px 1rem 70px">
      <tbody>
        ...
        <td><img data-id="react-email-img" alt="Plaid" src="https://easysign.rhinocerosoftware.co/images/logo.jpeg" width="70" style="display:block;outline:none;border:none;text-decoration:none;margin:0 auto" />
            ...
            <h1 data-id="react-email-heading" style="color:#000;display:inline-block;font-family:HelveticaNeue-Medium,Helvetica,Arial,sans-serif;font-size:16px;font-weight:300;line-height:24px;margin-bottom:0;margin-top:0;text-align:center">Mme / Mr <b>{{customerName}}</b>, votre document <b>{{documentName}}</b> vient d'obtenir toutes les signatures demandées</h1>
            <table align="center" width="100%" data-id="react-email-section" style="background:#d8f7c6;border-radius:4px;margin:16px auto 14px;vertical-align:middle;width:280px" border="0" cellPadding="0" cellSpacing="0" role="presentation">
              <tbody>
                <tr>
                  <td>
  ...
  async afterUpdate(event) {
    ...
    await strapi.entityService
      ...
      .then((result) => {
        ...
        signatories = result?.levelVersions?.filter(_target => {
    ...
    if (false && ProcessRecords.docUpdates?.some(target => target?.id === docData?.id && docData?.levelVersions?.every((_target, id) => _target?.signed === target?.versions[id]?.signed && _target?.rejectionMessage === target?.versions[id]?.rejectionMessage))) {
      console.log("prevented duplicate notifications loop", ProcessRecords.docUpdates);
      ...
      if (rejectorMan) {
        await Promise.allSettled(
          signatories?.map((target) => {
            ...
            IOServer?.io.to(OnlineUsers.getOne(target?.id?.toString())?.socket)?.emit("DOC_REJECTION", {
          ...
          .catch((error) => {
            console.log("an error has occured when sending notifications", error);
          });
      } else {
        if (versions?.every(_target => _target?.signed) && versions?.length > 1) {
          ...
          IOServer?.io.to(OnlineUsers.getOne(author?.id)?.socket)?.emit("DOC_COMPLETION", {
            documentName: result?.title,
            department: author?.department?.name
          })

          console.log("will send all notifications event", OnlineUsers.getAll());
          ...
          strapi.plugins["email"].services?.email
            .send({
              ...
              html: confirmTemplate
                ...
                ?.replaceAll("{{documentName}}", result?.title)?.replaceAll("{{filePath}}", downloadFilePath)
            })
            .then((results) => {
              console.log("sent email successfully", results?.response);
          ...
          IOServer?.io.to(OnlineUsers.getOne(nextSigner?.id)?.socket)?.emit("DOC_SIGNATURE", {
            documentName: result?.title,
