...
module.exports = {
  async afterUpdate(data) {
    ...
    await strapi.entityService
      ...
      .then(async (result) => {
        if (
          ProcessRecords.fileProcesses?.some(
            (target) =>
              target?.versionId === result?.id &&
              target?.fileId === result?.file?.id &&
              target?.signed === result?.signed &&
              target?.rejectionMessage === result?.rejectionMessage
          )
        ) {
          console.log(
            "prevented falling into infinite version after update hook"
          );
        } else {
          ...
          await strapi.entityService
            .update("api::document.document", result?.parentDocument?.id, {
