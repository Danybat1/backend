...
module.exports = {
  async index(ctx, next) {
    ...
    const wordFilePath = path.join(process.cwd(), "public", ctx?.request?.body?.data?.filePath);

    const outputFile = wordFilePath?.replace(".docx", ".pdf")?.replace(".doc", ".pdf")
    ...
    await fs.writeFile(outputFile, pdfBuf).then(data => {
    ...
    if (processed) {
      ...
      ctx.body = {
        code: 200,
        pdfFilePath: outputFile?.split(process.cwd())[1]?.replace("/public", "")
      };
    } else {
      ctx.status = 400;
